...
```bash
# Run all KMS-owned Mercedes PKI tests (standalone):
...
| **PKI-06** | Self-signed certificate prohibition | High | ✅ **New** (M-01) | `test_mercedes_pki.sh` M-01 — pathlen:0 verified on all issued intermediates |
...
| **PKI-04** | Zero-downtime trust bundle rotation | High | ✅ **New** (M-04) | `test_mercedes_pki.sh` M-04 — new CA key created, old CA continues signing during overlap |
...
| **PKI-09** | PAM integration for long-lived secrets | Medium | 🔶 Joint workshop | Requires live Segura instance; see `Mercedes.txt` I-3 for integration design |
| **PKI-10** | Service mesh SDS delivery | Medium | 🔶 Aembit/Envoy | Envoy SDS is the delivery layer; KMS provides the SVID via SPIRE |
| **PKI-11** | TLS 1.3 enforcement | High | ✅ **New** (M-02) | `test_mercedes_pki.sh` M-02 — TLS 1.2 curl → connection failure |
| **PKI-12** | Algorithm policy change without redeploy | High | ✅ **New** (M-03) | `test_mercedes_pki.sh` M-03 — second KMS with restricted allowlist, P-256 → 4xx, P-384 → 200 |
| **PKI-13** | Documented PKI responsibility split | High | 📄 Documentation | See `Mercedes.txt` PKI-2 section (written answer) |
...
| **PKI-17** | Trust re-establishment | High | ✅ **New** (M-05) | `test_mercedes_pki.sh` M-05 — AppRole secret_id expired → new secret_id → fresh login succeeds |
...
| OBS-05 | Issuance latency < 500 ms (NFR-2) | High | ✅ **New** (M-06) | `test_mercedes_pki.sh` M-06 — 5× timed `sign-intermediate` calls, each < 500ms |
...
| RES-08 | Backwards compatibility / phased migration | High | ✅ **New** (M-08) | `test_mercedes_pki.sh` M-08 — legacy + SPIFFE keys coexist in same KMS namespace |
...
| INFO-2 | Independent signing key lifecycle for message-signing keys? | **Yes.** KMS manages DPoP/message-signing key pairs via KMIP `CreateKeyPair` / `ReKeyKeyPair` / `Revoke` / `Destroy` on `/kmip/2_1`. These are completely separate from workload SVID transit keys. Demonstrated in `test_mercedes_pki.sh` M-07. |
...
|----------|-----------------|----------|
| M-01 | PKI-06 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-02 | PKI-11 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-03 | PKI-12 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-04 | PKI-04 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-05 | PKI-17 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-06 | OBS-05 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-07 | INFO-2 | `test_data/spire/setup/test_mercedes_pki.sh` |
| M-08 | RES-08 | `test_data/spire/setup/test_mercedes_pki.sh` |
